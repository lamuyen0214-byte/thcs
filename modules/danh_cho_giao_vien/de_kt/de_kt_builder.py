...
# =====================================================================
# # 2. NẠP ĐỘNG CƠ TỪ KIẾN TRÚC MỚI (GIỮ NGUYÊN 100%)
# =====================================================================
...
def render_de_kt_module(api_key=""):
    ...
    # 2. HÀNG 1: MENU ĐIỀU HƯỚNG CỐ ĐỊNH 4 CỘT (GIỮ NGUYÊN 100%)
    ...
    # 3. HÀNG 2: TỶ LỆ MỨC ĐỘ NHẬN THỨC CỐ ĐỊNH CỦA THẦY (GIỮ NGUYÊN 100%)
    ...
    if (nhan_biet + thong_hieu + van_dung + van_dung_cao) != 100:
        st.error(" Tổng tỷ lệ phần trăm mức độ nhận thức phải bằng 100%!")

    # 4. HÀNG 3: KHU VỰC TẢI FILE TÀI LIỆU VÀ GẮN KEY TĨNH CHO Ô NHẬP LIỆU CHỮ (GIỮ NGUYÊN 100%)
    ...
    # Chia đôi không gian màn hình thênh thang wide cho Trắc nghiệm và Tự luận (GIỮ NGUYÊN 100%)
    ...
    if activated:
        if not ten_bai.strip():
            st.warning(" Vui lòng nhập 'Tên bài kiểm tra / Đề số' trước khi khởi tạo.")
        else:
            # =====================================================================
            # BỘ ĐIỀU KHIỂN TRUNG TÂM MỚI (AI ENGINE - SỬA LỖI ĐỒNG BỘ ĐĂNG NHẬP)
            # =====================================================================
            # Thay vì gọi hàm rỗng không đồng bộ, ta ưu tiên sử dụng api_key truyền từ router xuống
            # Nếu api_key trống, ta fallback lấy từ get_api_key() để tương thích ngược.
            final_key = api_key if api_key and api_key.strip() else get_api_key()
            
            if not final_key:
                st.error(" Lỗi hệ thống: Chưa nhận diện được API Key cá nhân hợp lệ tại Sidebar.")
            ...
            with st.spinner("AI đang soạn đề thi bám sát chương trình sách Kết nối tri thức..."):
                ...
                if not file_context.strip(): 
                    file_context = f"Phạm vi chủ đề bài kiểm tra: {ten_bai}."
                
                # Hệ thống prompt tinh gọn và sắc bén (GIỮ NGUYÊN 100%)
                ...
                # Xác định mô hình tự động qua tên Selectbox (GIỮ NGUYÊN 100%)
                mode = "pro" if "Pro" in model_display_name or "mở rộng" in model_display_name.lower() else "flash"
                
                # GỌI ENGINE CỐT LÕI VỚI KEY LIÊN THÔNG ĐĂNG NHẬP
                result = run_ai_with_fallback(
                    prompt=prompt_de_kt, 
                    api_key=final_key, # Sử dụng key liên thông thiết bị
                    model_mode=mode
                )
    ...
    if exam_cache:
        ...
        if WordEngine:
            try: 
                ...
                st.error(f" Trình kết xuất file Word đang đồng bộ: {e}")
                
        # ÉP CỨNG BỘ 3 NÚT BẰNG BIẾN ĐỘC LẬP - HIỂN THỊ NGANG NHAU TĂM TẮP (GIỮ NGUYÊN 100%)
