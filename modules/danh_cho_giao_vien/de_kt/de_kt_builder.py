...
def render_de_kt_module():
    # 1. CẤU HÌNH CSS ĐỂ KHÓA BỐ CỤC CỐ ĐỊNH (THU NHỎ CHỮ ĐIỂM CHỐNG NHẢY DÒNG CHỮ)
    st.markdown("""
        <style>
...
# =====================================================================
    ...
    # 4. HÀNG 3: ĐÃ SỬA LỖI LAYOUTSMIXIN: Khai báo rõ 3 cột cho vùng uploader tải file
    ...
    # 5. HÀNG 4: ĐÃ SỬA LỖI LAYOUTSMIXIN: Khai báo rõ 3 cột chia đôi không gian màn hình làm việc trái/phải
    col_tn, spacer, col_tl = st.columns(3)
# =====================================================================
# FILE: modules/danh_cho_giao_vien/de_kt/de_kt_builder.py - ĐOẠN 3
# =====================================================================
    # --- CỘT TRÁI: TRẮNG NGHIỆM ĐỘNG GIAO DIỆN CỐ ĐỊNH VÀ SỬA ĐỐI SỐ COLUMNS ---
    with col_tn:
        tn_header = st.empty()
        st.write("")
    ...
    # --- CỘT PHẢI: TỰ LUẬN ĐỘNG GIAO DIỆN CỐ ĐỊNH VÀ SỬA ĐỐI SỐ COLUMNS ---
    with col_tl:
...
# =====================================================================
    # 7. SỰ KIỆN NÚT BẤM GỌI AI ĐÃ ĐỒNG BỘ ĐƯỜNG DẪN CONFIG ĐA CẤP CỦA THẦY
    col_btn_run, col_model_sel = st.columns(2)
    with col_model_sel:
        model_display_name = st.selectbox("Mô hình", ["3.1 Flash-Lite", "3.5 Flash", "3.1 Pro", "Tư duy mở rộng"], label_visibility="collapsed", index=0)
    with col_btn_run:
    ...
    if activated:
        if not ten_bai.strip():
            ...
            with st.spinner("AI đang soạn thảo ma trận đặc tả kỹ thuật và đề thi..."):
                ...
                if not file_context.strip(): file_context = f"Phạm vi chủ đề: {ten_bai}."

                # VÁ LUỒNG ĐA CẤP: Dò tìm file config/models.py bất chấp folder con main/ bẻ gãy lỗi ModuleNotFound
                try:
                ...
                try:
                    ...
                    system_instruction = f"Bạn là Chuyên gia khảo thí của Bộ GD&ĐT Việt Nam. Bộ sách độc tôn năm 2026: 'Kết nối tri thức với cuộc sống'. Hãy soạn ma trận dạng bảng, đặc tả và đề thi môn {mon_hoc} {lop} bám sát chủ đề: {ten_bai}. Tỷ lệ: Nhận biết {nhan_biet}%, Thông hiểu {thong_hieu}%, Vận dụng {van_dung}%, Vận dụng cao {van_dung_cao}%."
    ...
    # 8. BỘ 3 NÚT CHỨC NĂNG KẾT XUẤT HỒ SƠ CỔ ĐỊNH NGOÀI GIAO DIỆN CHUẨN ĐỒ HỌA
    st.markdown("---")
    ...
    # LUÔN LUÔN HIỂN THỊ KHUNG 3 NÚT BẤM ĐỐI XỨNG THEO ĐÚNG PHƯƠNG ÁN SƯ PHẠM CỦA THẦY
    ...
    with col_download:
        if word_file is not None and exam_cache is not None:
            ...
            st.download_button(label="📄 Tải file về máy", data=word_file, file_name=f"Bo_De_Kiem_Tra_{saved_title}.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document", use_container_width=True, key="btn_dl_word_de_kt")
        else:
