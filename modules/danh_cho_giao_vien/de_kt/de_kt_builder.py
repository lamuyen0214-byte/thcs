...
# =====================================================================
# 1. ĐỊNH VỊ ĐƯỜNG DẪN GỐC (GIỮ NGUYÊN)
# =====================================================================
...
# =====================================================================
# 3. HÀM RENDER CHÍNH (BẮT ĐẦU CÁCH LY GIAO DIỆN)
# =====================================================================
def render_de_kt_module(api_key=""):
    # Bao bọc toàn bộ trong 1 container để giữ bố cục không bị tan nát
    with st.container():
        st.markdown("""
        <style>
        ...
        with col_tl:
            ...
            for i in range(1, int(so_cau_tl) + 1):
                ...
                with c2: diem = st.number_input("Điểm", value=1.0, step=0.25, format="%.2f", key=f"diem_tl_{i}_de_kt_k", label_visibility="collapsed")
        ...
        with col_req: bam_sat = st.checkbox("Bám sát nội dung đề cương/ma trận tải lên", value=True, key="chk_bam_sat_de_kt")
        ...
        with col_model_sel: model_display_name = st.selectbox("Mô hình", ["3.1 Flash-Lite", "3.5 Flash", "3.1 Pro", "Tư duy mở rộng"], label_visibility="collapsed", index=0, key="sb_model_ai_de_kt_run")
        with col_btn_run: activated = st.button(" TỰ ĐỘNG KHỞI TẠO MA TRẬN VÀ ĐỀ THI", type="primary", use_container_width=True, key="btn_submit_run_de_kt")

        if activated:
            if not ten_bai.strip(): st.warning(" Vui lòng nhập 'Tên bài kiểm tra / Đề số' trước khi khởi tạo.")
            else:
                final_key = api_key if api_key and api_key.strip() else get_api_key()
                if not final_key: st.error(" Lỗi hệ thống: Chưa nhận diện được API Key."); st.stop()
                
                with st.spinner("AI đang soạn đề..."):
                    file_context = "Phạm vi: " + ten_bai
                    system_instruction = f"Bạn là Chuyên gia khảo thí. Soạn đề {mon_hoc} {lop}. Tỷ lệ nhận thức: NB {nhan_biet}%, TH {thong_hieu}%, VD {van_dung}%, VDC {van_dung_cao}%. {yeu_cau_khac}"
                    prompt_de_kt = system_instruction + "\n\n[TÀI LIỆU]:\n" + file_context
                    mode = "pro" if "Pro" in model_display_name else "flash"
                    
                    result = run_ai_with_fallback(prompt=prompt_de_kt, api_key=final_key, model_mode=mode)
                    
                    if result.get("success"):
                        st.session_state['current_exam_data'] = {
                            "is_khbd": False, "type": hinh_thuc, "ten_bai_save": str(ten_bai),
        ...
        if exam_cache:
            with st.expander("Xem kết quả đề thi", expanded=True):
            ...
            if WordEngine:
                try: 
                    ...
                    st.download_button("Tải file Đề thi (Word)", data=word_file, file_name="De_Kiem_Tra.docx", use_container_width=True)
                except Exception as e: st.error(f"Lỗi xuất file: {e}")

            if st.button("Xóa dữ liệu đề", use_container_width=True):
