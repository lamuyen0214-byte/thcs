# =====================================================================
# FILE: modules/danh_cho_giao_vien/organization/homeroom_builder.py
# =====================================================================
...
def render_homeroom_module():
    ...
    nghiep_vu = st.radio(
        ...
        ["📝 Nhận xét học bạ", "👥 Kịch bản họp phụ huynh", "🧩 Xử lý tình huống sư phạm"],
        horizontal=True
    )
    ...
    if nghiep_vu == "📝 Nhận xét học bạ":
        ...
        noi_dung_input = st.text_area(
            "Đặc điểm học sinh", 
            placeholder="Ví dụ: Lực học KHTN tốt, tích cực tham gia các dự án khoa học. Tuy nhiên còn trầm tính, cần chủ động hơn khi làm việc nhóm. Rất hòa đồng và luôn sẵn sàng hỗ trợ các bạn có nhu cầu học tập đặc biệt trong lớp...", 
...
"""
        ...
        noi_dung_input = st.text_area(
            "Trọng tâm cuộc họp", 
            placeholder="Ví dụ: Họp phụ huynh đầu năm Lớp 9, phổ biến kế hoạch học tập, định hướng phân luồng ôn thi vào lớp 10, và kêu gọi sự đồng hành của phụ huynh trong các hoạt động ngoại khóa...", 
            height=100, label_visibility="collapsed"
        )
        prompt_he_thong = f"""
...
Yêu cầu: Lời văn trang trọng, gắn kết, thể hiện sự chuyên nghiệp và phối hợp chặt chẽ giữa Gia đình - Nhà trường. Trình bày bằng Markdown.
"""
        ...
        noi_dung_input = st.text_area(
            "Tình huống", 
            placeholder="Ví dụ: Trong lớp có một nhóm học sinh quá mải mê làm dự án nghiên cứu khoa học mà lơ là các môn học khác; hoặc học sinh khuyết tật học hòa nhập bị các bạn trong lớp trêu chọc...", 
            height=100, label_visibility="collapsed"
        )
        prompt_he_thong = f"""
...
3. Biện pháp giáo dục và hỗ trợ bền vững (Dài hạn) đảm bảo môi trường lớp học an toàn, hòa nhập và tôn trọng sự khác biệt.
4. Gợi ý mẫu lời thoại giáo viên dùng để nói chuyện trực tiếp với học sinh/phụ huynh.
Yêu cầu: Giải pháp phải mang tính nhân văn, tuân thủ nguyên tắc sư phạm và bám sát thực tiễn giáo dục. Trình bày bằng Markdown.
"""
