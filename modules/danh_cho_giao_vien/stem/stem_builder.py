...
def render_stem_module():
    ...
    if submit_btn:
        if chu_de.strip() == "":
            ...
            with st.spinner("AI đang phân tích tài liệu và xây dựng kịch bản bài dạy..."):
                # Gọi PromptManager để trích xuất hệ thống câu lệnh mồi chuẩn 5E
                sys_inst, prompt = PromptManager.get_stem_prompt(
                    chu_de, mon_hoc, lop, yeu_cau_dac_biet
                )
                
                # Truyền tham số sang tầng mô hình sinh nội dung kịch bản
                result = gemini_instance.generate_content(prompt, sys_inst)
                
                if result:
                    st.success("✅ Đã tạo xong kịch bản bài dạy!")
                    with ctk.CTkFrame(self, fg_color="transparent") if 'current_stem_content' not in st.session_state else st.container():
