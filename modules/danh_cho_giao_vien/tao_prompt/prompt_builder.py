...
def render_prompt_module(api_key=""):
    st.markdown("""
    <style>
    ...
    # 1. KHUNG NHẬP LIỆU Ý TƯỞNG (GIỮ NGUYÊN 100%)
    ...
    # 2. XỬ LÝ AI - TỐI ƯU PROMPT (SỬA ĐỔI DUY NHẤT: ĐỒNG BỘ MÃ KHÓA BIẾN MÔI TRƯỜNG ĐĂNG NHẬP)
    if st.button(" TỐI ƯU HÓA SIÊU CÂU LỆNH (PROMPT)", type="primary", use_container_width=True):
        if not y_tuong_tho.strip():
            st.warning(" Thầy/Cô vui lòng nhập ý tưởng thô trước khi tối ưu hóa.")
            return
            
        # =====================================================================
        # GỌI BỘ ĐIỀU KHIỂN TRUNG TÂM & KIỂM TRA CHỐNG LỖI NONETYPE (ĐÃ TRUYỀN THAM SỐ)
        # =====================================================================
        # Chuyển tiếp tham số api_key nhận từ file chạy chính xuống hàm sinh client
        ...
        with st.spinner(" Hệ thống đang phân tích và tái cấu trúc câu lệnh..."):
            try:
                # Hệ thống cấu trúc tạo Siêu câu lệnh (GIỮ NGUYÊN TOÀN VẸN 100%)
                prompt_he_thong = f"""
...
3. **Mẹo sử dụng:** Đưa ra 1-2 mẹo nhỏ để người dùng điền thêm biến số nếu cần.
 """
    ...
    # 3. KẾT XUẤT WORD (GIỮ NGUYÊN TOÀN VẸN BỐ CỤC ĐỐI XỨNG CỦA THẦY)
    st.markdown("---")
