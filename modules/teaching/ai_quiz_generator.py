...
# 1. Hàm tự động dò tìm model (Sửa lỗi 404)
def get_stable_model(api_key):
    ...
    try:
        ...
        # Liệt kê các model khả dụng và tự chọn model hỗ trợ tạo nội dung
        models = genai.list_models()
        available = [m.name for m in models if 'generateContent' in m.supported_generation_methods]
        
        # Ưu tiên các dòng Flash/Pro mới nhất
        for name in ["gemini-1.5-flash", "gemini-1.5-pro", "gemini-2.0-flash-lite"]:
            if name in available:
                return genai.GenerativeModel(name)
        
        # Fallback nếu không khớp tên ưu tiên
        if available:
            return genai.GenerativeModel(available[0])
            
        st.error("Không tìm thấy model AI khả dụng.")
        return None
    except Exception as e:
        st.error(f"Lỗi hệ thống AI: {e}")
        return None

# 2. Giao diện chức năng Quiz Generator
