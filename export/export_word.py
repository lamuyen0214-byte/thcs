...
class WordExportEngine:
    ...
    def build_matrix_table(doc, exam_data):
        """Vẽ bảng ma trận trộn ô 2 tầng Công văn 799 chuẩn xác chỉ số hàng ô"""
        # Bảng gồm 5 hàng (2 hàng tiêu đề, 2 hàng dữ liệu, 1 hàng tổng) và 11 cột
        table = doc.add_table(rows=5, cols=11)
        table.style = 'Table Grid'
        table.autofit = False
        
        col_widths = [Inches(0.4), Inches(1.6), Inches(1.6), Inches(0.4), Inches(0.4), Inches(0.4), 
                      Inches(0.4), Inches(0.4), Inches(0.4), Inches(0.4), Inches(0.7)]
        ...
        # SỬA LỖI CÚ PHÁP: Trộn ngang tầng 1 và điền ký hiệu TN/TL tầng 2 cho các nhóm năng lực
        for i in:
            r0[i].merge(r0[i+1])
            r1[i].text, r1[i+1].text = "TN", "TL"
            
        # Cột VDC (chỉ có tự luận) và cột Tổng
        r1[9].text = "TL"
        r1[10].text = "Điểm"
        
        # SỬA LỖI CÚ PHÁP: Trộn dọc cô lập cho các cột STT, Chủ đề, Nội dung
        for idx in:
            r0[idx].merge(r1[idx])
            
        # Đổ màu nền và khóa font chữ đậm tiêu đề cho 2 hàng đầu
        ...
        # Dữ liệu bắt đầu đổ từ hàng chỉ số 2 trong bảng
        matrix_rows = [
            ...
            ["", "TỔNG CỘNG ĐỀ KIỂM TRA ĐẠT CHUẨN", "", f"{c1}", "0", "0", f"{c2}", "2", "1", "1", "10.0 điểm"]
        ]
        ...
        for r, data in enumerate(matrix_rows, start=2):
            for c, text in enumerate(data):
                ...
                cell.text = text
                cell.paragraphs[0].paragraph_format.space_after = Pt(3)
                if r == 4:  # Đổ màu hàng tổng cộng cuối cùng
                    bg_cell(cell, "EBF5FB")
    ...
    def build_specification_table(doc, exam_data):
        """Xây dựng bảng đặc tả ma trận đề thi phân hóa năng lực"""
    ...
    def export_to_word(data_cache):
        """Hàm điều phối hợp nhất kết xuất tệp Word hành chính khít lề 3pt dòng"""
        doc = docx.Document()
        
        # Cấu hình căn lề chuẩn văn bản hành chính Việt Nam (Top/Bottom 2cm, Left 3cm, Right 2cm)
        ...
        if data_cache.get("is_khbd") == True:
            ...
            # Tiến hành chèn bảng ma trận và đặc tả trước nội dung đề thi
        ...
        # Làm sạch các ký hiệu markdown thô trước khi nạp vào paragraph trong Word
        ...
        for line in clean_content.split('\n'):
            if line.strip():
                ...
                # Tự động in đậm tiêu đề phần lớn hoặc các đầu mục quan trọng
                strip_line = line.strip()
                if strip_line.startswith("I.") or strip_line.startswith("II.") or "Hoạt động" in strip_line or "MỤC TIÊU" in strip_line or "TIẾN TRÌNH" in strip_line or strip_line.startswith("PHẦN") or strip_line.startswith("Câu "):
                    if p_item.runs: 
